...
public class FilmFrame extends JFrame {
    ...
    class KonyvTableCellRenderer implements TableCellRenderer {
        ...
        public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int col) {
            ...
            if((boolean)data.getValueAt(table.getRowSorter().convertRowIndexToModel(row), 3)){
                data.setValueAt(LocalDate.now().toString(), table.getRowSorter().convertRowIndexToModel(row), 4);
                data.setValueAt(LocalDate.now().plusDays(20).toString(), table.getRowSorter().convertRowIndexToModel(row), 5);
            }
    ...
    public FilmFrame(ArrayList<Filmografia> kell){
